...
icodir="/home/saiddis/.config/hypr/icons/media"
step=${VOLUME_STEPS:-5}
...
notify_vol() {
    ...
    [[ "${isNotify}" == true ]] && notify-send -r 8 -t 2000 -i "${ico}" "${vol}${bar}" "${nsink}"
}
...
notify_mute() {
    ...
    if [ "${mute}" == "true" ]; then
        [[ "${isNotify}" == true ]] && notify-send -r 8 -t 2000 -i "${icodir}/muted-${dvce}.svg" "muted" "${nsink}"
    else
        [[ "${isNotify}" == true ]] && notify-send -r 8 -t 2000 -i "${icodir}/unmuted-${dvce}.svg" "unmuted" "${nsink}"
    fi
}
